...
class TestCLI():
    ...
    def test_02_create_custom_repos():
        ...
        cmd = "rhua rhui-manager repo create_custom --repo_id yay --display-name Yay"
    ...
    def test_04_check_custom_repos():
        ...
        # test an unrecognized option
        Expect.expect_retval(RHUA, "rhua rhui-manager repo list --redhat", 254)
    ...
    def test_06_upload_remote_rpms(self):
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager packages remote --repo_id a --url ftp://b.be/c/ --def"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_07_check_packages(self):
        ...
        nose.tools.eq_(package_lists[0], Util.get_rpms_in_dir(RHUA, DATADIR_HOST))
        ...
        # test an unrecognized option
        Expect.expect_retval(RHUA, "rhua rhui-manager packages list --repo_id a --meow", 254)
    ...
    def test_08_upload_certificate():
        ...
        Expect.expect_retval(RHUA,
                             "rhua rhui-manager cert upload --cert /tmp/c.crt --lock thedoor",
                             254)
    ...
    def test_09_check_certificate_info(self):
        ...
        # test an unrecognized option
        Expect.expect_retval(RHUA, "rhua rhui-manager cert info --debug", 254)
    ...
    def test_11_check_unused_product(self):
        ...
        # test an unrecognized option
        Expect.expect_retval(RHUA, "rhua rhui-manager repo unused --by-repo_id", 254)
    ...
    def test_12_add_rh_repo_by_id(self):
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager repo add_by_repo --repo_ids boo --names Boo"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_13_add_rh_repo_by_product(self):
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager repo add --product_name All --sync_now"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_14_repo_list(self):
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager repo list --ids_only --delimeter ,"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_15_start_syncing_repo(self):
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager repo sync --repo_id myrepo --progress=dot"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_18_list_labels(self):
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager client labels --json"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_19_generate_certificate(self):
        ...
        RHUIManagerCLI.client_cert(RHUA,
                                   ...
                                   TMPDIR)
        # test an unrecognized option
        cmd = "rhua rhui-manager client cert --auto"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_21_check_stray_custom_repo(self):
        ...
        _, stdout, _ = RHUA.exec_command(f"cat {TMPDIR_HOST}/{CLI_CFG[0]}-extensions.txt")
    ...
    def test_22_create_cli_config_rpm():
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager client rpm --wizard"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_23_ensure_gpgcheck_config(self):
        ...
        remote_repo_file = f"{TMPDIR_HOST}/{CLI_CFG[0]}-{CLI_CFG[1]}/build/BUILD/" + \
    ...
    def test_26_create_acs_config_rpm(self):
        ...
        RHUIManagerCLI.client_content_source(RHUA,
                                             ...
                                             TMPDIR)
        # check that
        cmd = f"rpm2cpio {TMPDIR_HOST}/{name}-2.0/build/RPMS/noarch/{name}-2.0-1.noarch.rpm | " + \
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager client content_source --rpm_name cs --dir / --rpm_epoch 2"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_27_create_acs_config_json(self):
        ...
        RHUIManagerCLI.client_acs_config(RHUA,
                                         ...
                                         ssl_ca_file)
        # check that
        _, stdout, _ = RHUA.exec_command(f"cat {TMPDIR_HOST}/acs-configuration.json")
        ...
        nose.tools.ok_(Util.cert_expired(RHUA, f"{TMPDIR_HOST}/acs-entitlement.crt", 90000))
        # but not 23 hours from now?
        nose.tools.ok_(not Util.cert_expired(RHUA, f"{TMPDIR_HOST}/acs-entitlement.crt", 82800))
        # whether the custom CA cert was used
        _, stdout, _ = RHUA.exec_command(f"rhua cat {ssl_ca_file}")
        ...
        _, stdout, _ = RHUA.exec_command(f"rhua cat {default_ca_path}")
        ...
        nose.tools.assert_raises(NoValidEntitlementsProvided,
                                 ...
                                 TMPDIR)
        # test an unrecognized option
        cmd = "rhua rhui-manager client acs_config --directory /"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_29_upload_incompat_cert():
        ...
        cert = join(DATADIR_HOST, CERTS["incompatible"])
    ...
    def test_30_remove_package():
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager packages remove --repo_id z --package a-1-2.noarch.rpm --brutef"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_38_resync_no_warning():
        ...
        logs_host = "/var/lib/rhui/log"
        # for RHBZ#1506872
        Expect.expect_retval(RHUA, f"grep -r 'pulp.*metadata:WARNING' {logs_host}/*", 1)
        # for RHBZ#1579294
        Expect.expect_retval(RHUA, f"grep -r 'pulp.*publish:WARNING' {logs_host}/*", 1)
        # for RHBZ#1487523
        Expect.expect_retval(RHUA, f"grep -r 'pulp.*Purging duplicate NEVRA can' {logs_host}/*", 1)

    @staticmethod
    def test_39_list_repos():
        '''get a list of available repos for further examination'''
        cmd = f"rhua rhui-manager repo unused > {TMPDIR_HOST}/repos.out 2> {TMPDIR_HOST}/repos.err"
    ...
    def test_40_check_iso_repos():
        ...
        # for RHBZ#1199426
        Expect.expect_retval(RHUA,
                             f"egrep 'Containers|Images|ISOs|Kickstart' {TMPDIR_HOST}/repos.out", 1)

    @staticmethod
    def test_41_check_pygiwarning():
        '''check if PyGIWarning was not issued'''
        # for RHBZ#1450430
        Expect.expect_retval(RHUA, f"grep PyGIWarning {TMPDIR_HOST}/repos.err", 1)
    ...
    def test_43_repo_export(self):
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager repo export --repo_id bob --log /tmp/exports"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_44_upload_semi_bad_cert(self):
        ...
        cert = join(DATADIR_HOST, CERTS["partial"])
        if Util.cert_expired(RHUA, cert):
            raise nose.exc.SkipTest("The given certificate has already expired.")
        RHUIManagerCLI.cert_upload(RHUA, cert)
        # the RHUI log must contain the fact that an invalid path was found in the cert
        Expect.ping_pong(RHUA, "tail /var/lib/rhui/root/.rhui/rhui.log", "Invalid entitlement path")
        RHUIManager.remove_rh_certs(RHUA)
    ...
    def test_45_upload_empty_cert():
        ...
        # for RHBZ#1497028
        cert = join(DATADIR_HOST, CERTS["empty"])
        if Util.cert_expired(RHUA, cert):
            raise nose.exc.SkipTest("The given certificate has already expired.")
        try:
            RHUIManagerCLI.cert_upload(RHUA, cert)
    ...
    def test_47_add_by_file():
        ...
        #good_file_host = "/var/lib/rhui" + IMPORT_REPO_FILES["good"]
        expected_repo_ids = sorted(Helpers.get_repos_from_yaml(RHUA, IMPORT_REPO_FILES["good"]))
        # upload the cert and try adding the repos from the file, sync them at the same time
        RHUIManagerCLI.cert_upload(RHUA, join(DATADIR, CERTS["normal"]))
        ...
        # test an unrecognized option
        cmd = "rhua rhui-manager repo add_by_file --file repos.yml --sync"
        Expect.expect_retval(RHUA, cmd, 254)
    ...
    def test_48_rhui_scripts():
        ...
        for script, log in zip(scripts, logs):
            Expect.expect_retval(RHUA, f"rhua {script} --config {bad_config}", 1)
            Expect.ping_pong(RHUA,
                             f"rhua tail {log}",
                             "(pulp_api_url|cert_dir|cert_redhat_dir) is absent or invalid")
            Expect.expect_retval(RHUA, f"rhua {script} --sync-config {bad_sync_config}", 1)
            Expect.ping_pong(RHUA, f"rhua tail -2 {log}", "username is absent or invalid")

    @staticmethod
    def test_49_rhui_manager_help():
        '''test help handling in rhui-manager'''
        for arg in ["-h", "--help"]:
            Expect.ping_pong(RHUA, f"rhua rhui-manager {arg}", "Usage:.*Options:.*Commands:")
            # -h doesn't work with 1-level subcommands like --help, skip it
            if arg != "-h":
                cmd = f"rhua rhui-manager cert {arg}"
                Expect.ping_pong(RHUA, cmd, "upload.*uploads.*info.*display")
            Expect.ping_pong(RHUA, f"rhua rhui-manager cert info {arg}", "info: display")
        # test an unrecognized option
        Expect.expect_retval(RHUA, "rhua rhui-manager --signout", 2)

    @staticmethod
    def test_50_sync_invalid_repo():
