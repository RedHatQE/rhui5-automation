...
def test_04_create_custom_repo():
    ...
    RHUIManagerRepo.add_custom_repo(RHUA,
                                    REPO,
                                    redhat_gpg=False,
                                    custom_gpg=f"{CUSTOM_RPMS_DIR}/{KEY_FILENAME}")
...
def test_07_generate_ent_cert():
    ...
    '''
    RHUIManagerClient.generate_ent_cert(RHUA, [REPO], REPO, MYDIR)
...
def test_09_install_conf_rpm():
    ...
    Util.install_pkg_from_rhua(RHUA,
                               CLI,
                               f"{MYDIR_HOST}/{REPO}-2.0/build/RPMS/noarch/{REPO}-2.0-1.noarch.rpm")
...
def test_99_cleanup():
    ...
    Expect.expect_retval(RHUA, f"rm -rf {MYDIR_HOST}/{REPO}*")
    if not getenv("RHUISKIPSETUP"):
