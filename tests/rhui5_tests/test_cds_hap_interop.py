...
def test_99_cleanup():
    ...
    Expect.expect_retval(RHUA,
                         "rhua test -f /lib/systemd/system/pulpcore-resource-manager.service",
                         1)
    # and finally check if the containers are gone from the other nodes
    Expect.expect_retval(CDS, "cds uname", 125)
    Expect.expect_retval(HAPROXY, "ha uname", 125)
