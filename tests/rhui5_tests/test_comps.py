...
TEST_DIR = "/root/test_files/comps"
TEST_DIR_HOST = "/var/lib/rhui" + TEST_DIR
...
class TestCompsXML():
    ...
    def test_04_create_cli_config_rpms(self):
        ...
        for repo in self.test_repos:
            RHUIManagerCLI.client_rpm(RHUA, [repo], [repo], "/root")
    ...
    def test_05_install_conf_rpm(self):
        ...
        Util.install_pkg_from_rhua(RHUA,
                                   CLI,
                                   f"/var/lib/rhui/root/{self.test_repos[0]}-2.0/build/RPMS/noarch/"
    ...
    def test_06_check_groups(self):
        ...
        original_comps_xml_host = f"{TEST_DIR_HOST}/{self.test_repos[0]}/comps.xml"
        groups_in_xml = Yummy.comps_xml_grouplist(RHUA, original_comps_xml_host)
    ...
    def test_08_install_conf_rpm(self):
        ...
        Util.install_pkg_from_rhua(RHUA,
                                   CLI,
                                   f"/var/lib/rhui/root/{self.test_repos[1]}-2.0/build/RPMS/noarch/"
    ...
    def test_09_check_groups(self):
        ...
        groups_on_client = Yummy.grouplist(CLI)
        original_comps_xml_host = f"{TEST_DIR_HOST}/{self.test_repos[1]}/comps.xml"
        groups_in_xml = Yummy.comps_xml_grouplist(RHUA, original_comps_xml_host)
        nose.tools.eq_(groups_on_client, groups_in_xml)
    ...
    def test_12_additional_group(self):
        ...
        groups_on_client = Yummy.grouplist(CLI)
        original_comps_xml_host = f"{TEST_DIR_HOST}/{repo}/comps.xml"
        groups_in_xml = Yummy.comps_xml_grouplist(RHUA, original_comps_xml_host)
    ...
    def test_13_big_comps(self):
        ...
        original_comps_xml_host = f"{TEST_DIR_HOST}/{self.other_repos['big']}/comps.xml"
        original_comps_xml = f"{TEST_DIR}/{self.other_repos['big']}/comps.xml"
        original_groups = Yummy.comps_xml_grouplist(RHUA, original_comps_xml_host, False)
    ...
    def test_15_gzip(self):
        ...
        original_comps_xml_host = f"{TEST_DIR_HOST}/{self.other_repos['zip']}/comps.xml"
        original_groups = Yummy.comps_xml_grouplist(RHUA, original_comps_xml_host, False)
        # prepare a temporary file and compress the original comps into it
        compressed_comps_xml = Util.mktemp_remote(RHUA, ".xml.gz")
        Expect.expect_retval(RHUA, f"gzip -c {original_comps_xml_host} > {compressed_comps_xml}")
    ...
    def test_99_cleanup(self):
        ...
        for repo in self.test_repos:
            ...
            Expect.expect_retval(RHUA, f"rm -rf /var/lib/rhui/root/{repo}*")
