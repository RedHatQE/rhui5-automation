...
WORKDIR = f"/root/{NAME}"
WORKDIR_HOST = f"/var/lib/rhui{WORKDIR}"
...
class TestSyncPolicies():
    ...
    def test_05_set_non_default_policy():
        ...
        Config.set_sync_policy(RHUA, default_sync_policy=POLICIES["nondefault"])
    ...
    def test_09_readd_repos_non_default_policies(self):
        """remove the repos, tweak the configuration, re-add the repos, and sync them"""
        ...
        Config.set_sync_policy(RHUA, default_sync_policy=POLICIES["default"], backup=False)
        # then set the on-demand regex to debug and source repos
        Config.set_sync_policy(RHUA, on_demand_repoid_regex="debug|source", backup=False)
    ...
    def test_10_check_regular_repo_default(self):
        """examine the regular repo and check its default sync policy"""
        remote_data = PulpAPI.get_remote(RHUA, self.regular_repo)
        actual_policy = remote_data["policy"]
        nose.tools.eq_(actual_policy, POLICIES["default"])
    ...
    def test_13_create_install_cli_config_rpm(self):
        ...
        path = f"{WORKDIR_HOST}/{NAME}-1/build/RPMS/noarch/{NAME}-1-1.noarch.rpm"
        Util.install_pkg_from_rhua(RHUA, CLI, path)
