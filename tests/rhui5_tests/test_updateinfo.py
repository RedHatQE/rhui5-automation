...
TMPDIR = "/root/test_updateinfo"
DATADIR_HOST = "/var/lib/rhui" + DATADIR
TMPDIR_HOST = "/var/lib/rhui" + TMPDIR

class TestUpdateInfo():
    '''
       class for client tests using the updateinfo repodata
    '''
    ...
    def test_04_add_repo(self):
        ...
        try:
            if self.test["gpg_key"] == "nokey":
                ...
                custom_gpg = f"{DATADIR}/{self.test['repo_id']}/{self.test['gpg_key']}"
            redhat_gpg = False
        except KeyError:
            custom_gpg = None
            redhat_gpg = True
    ...
    def test_06_import_updateinfo(self):
        ...
        RHUIManagerCLI.repo_add_errata(RHUA,
                                       self.test["repo_id"],
                                       f"{DATADIR}/{self.test['repo_id']}/updateinfo.xml.gz")
    ...
    def test_08_create_cli_rpm(self):
        ...
        RHUIManagerClient.create_conf_rpm(RHUA,
                                          TMPDIR,
                                          f"{TMPDIR}/{self.test['repo_id']}.crt",
                                          f"{TMPDIR}/{self.test['repo_id']}.key",
                                          self.test["repo_id"])
    ...
    def test_09_install_conf_rpm(self):
        ...
        Util.install_pkg_from_rhua(RHUA,
                                   CLI,
                                   f"{TMPDIR_HOST}/{self.test['repo_id']}-2.0/build/RPMS/noarch/" +
    ...
    def test_12_compare_n_of_updates(self):
        ...
        _, stdout, _ = RHUA.exec_command(f"zgrep -o '{errata_pattern}' " +
                                         f"{DATADIR_HOST}/{self.test['repo_id']}" +
                                         "/updateinfo.xml.gz " +
                                         "| sort -u")
    ...
    def test_13_uncompressed_xml(self):
        ...
        cmd = "tail -1 /var/lib/rhui/root/.rhui/rhui.log | grep 'Import of err.*was successful'"
