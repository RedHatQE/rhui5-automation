...
def test_07_reinstall_all():
    ...
    '''
    # first, stop the CDS container
    stop_cmd = "systemctl stop rhui_cds"
    check_cmd = "cds test -f /etc/nginx/conf.d/ssl.conf"
    for cds_conn in CDS:
        Expect.expect_retval(cds_conn, stop_cmd)
        Expect.expect_retval(cds_conn, check_cmd, 125)
    ...
    # check if the CDS container was restarted after the reinstallation
    for cds_conn in CDS:
        Expect.expect_retval(cds_conn, check_cmd)
...
def test_20_check_cleanup():
    ...
    dirty_hosts["web"] = [cds.hostname for cds in CDS if Helpers.check_service(cds, service, "cds")]
    dirty_hosts["mount"] = [cds.hostname for cds in CDS if Helpers.check_mountpoint(cds, RHUI_ROOT)]

    if dirty_hosts["web"]:
        errors.append("nginx is still running on {dirty_hosts['web']}")
