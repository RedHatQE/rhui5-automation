...
def test_07_delete_nonexisting_cds():
    '''
        try deleting an untracked CDS, should be rejected (by rhui5_tests_lib)
    '''
...
def test_14_autoheal():
    ...
    _, stdout, _ = CDS[0].exec_command("cds pgrep -f gunicorn")
    ...
    # kill them all
    Expect.expect_retval(CDS[0], "cds killall gunicorn")
    # wait a little bit
    time.sleep(2)
...
def test_15_check_ansible_warnings():
    ...
    '''
    Expect.expect_retval(RHUA, "grep -i warning /var/lib/rhui/log/rhua_ansible.log", 1)
