...
CONF_RPM_PATH_HOST = f"{RPM_DIR_HOST}/{CONF_RPM_NAME}-{CONF_RPM_VERSION}/build/RPMS/noarch/" \
...
class TestClient():
    ...
    def test_07_create_cli_rpm():
        ...
        RHUIManagerClient.create_container_conf_rpm(RHUA,
                                                    ...
                                                    CONF_RPM_VERSION,
                                                    CONF_RPM_RELEASE)
        Expect.expect_retval(RHUA, f"test -f {CONF_RPM_PATH_HOST}")

    @staticmethod
    def test_08_install_cli_rpm():
        """install the client configuration RPM"""
        Util.install_pkg_from_rhua(RHUA, CLI, CONF_RPM_PATH_HOST)
