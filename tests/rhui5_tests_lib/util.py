...
class Util():
    ...
    def get_saved_password(connection,
                           creds_file="/var/lib/rhui/config/rhua/rhui-subscription-sync.conf"):
        '''
    ...
    @staticmethod
    def mktemp_remote(connection, ext=""):
        '''
        create a temporary file on the remote share (using the remote host) and return its path
        '''
        _, stdout, _ = connection.exec_command(f"mktemp -p {RHUI_ROOT} XXXX{ext}")
        path = stdout.read().decode().strip()
        Expect.expect_retval(connection, "chmod a+r " + path)
        return path
    ...
    def is_logged_in(connection):
        ...
        '''
        cmd = "test -f /var/lib/rhui/root/.rhui/http-localhost:24817/cookies.txt"
